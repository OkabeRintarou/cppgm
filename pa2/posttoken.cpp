...
struct PostTokenizer {
  ...
  void process(const PPToken &token) {
    PPTokenType type = token.type;
    ETokenType etype;

    if (type == PPTokenType::Tk_OpOrPunc) {
      auto it = StringToTokenTypeMap.find(token.data);
      if (it == StringToTokenTypeMap.end()) {
        ASSERT(false, "unexpected operator or punctuator");
      }
      output.emit_simple(token.data, it->second);
    } else if (type == PPTokenType::Tk_Identifier) {
      auto it = StringToTokenTypeMap.find(token.data);
      if (it != StringToTokenTypeMap.end()) {
        // keyword
        output.emit_simple(token.data, it->second);
    ...
    } else if (type == PPTokenType::Tk_CharacterLiteral) {
    } else if (type == PPTokenType::Tk_UdCharacterLiteral) {
...
int main() {
  ...
  //freopen("/home/syl/git/myproject/cppgm/pa2/tests/100-simple.t", "r", stdin);
